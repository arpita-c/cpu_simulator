...
public class AllMyStages {
    ...
    static class Decode extends PipelineStageBase<FetchToDecode,DecodeToExecute> {
        ...
        public void compute(FetchToDecode input, DecodeToExecute output) {
            ...
            if(!opcode.isBranch()) {
                if (source1IsRegister && source1RegisterInvalid) {
                         isFutureStageStalled = true; }
                else { if(source1IsRegister) {
                            ins.getSrc1().setValue(source1_globalregister_val);
                            ins.setSrc1(source1);
                     }
                }

                if (source2IsRegister && source2RegisterInvalid) {
                    isFutureStageStalled = true; }
                else{
                    if(source2IsRegister)
                    {
                        ins.getSrc2().setValue(source2_globalregister_val);
                        ins.setSrc2(source2);
                    }
                    }
                ...
                if(operand0IsRegister && operand0IsSource) {
                    if(operand0RegisterInvalid) {
                            isFutureStageStalled = true;
                        }
                    else{
                        ins.getOper0().setValue(operand0_globalregister_val);
                        ins.setOper0(operand0); }
                 }
                 else { if(operand0IsRegister)
                        globals.register_invalid[operand0RegisterNumber] = true;
                 }
