...
//                globals.setClockedProperty("branch_state_decode", GlobalData.BRANCH_STATE_NULL);
            ...
            if (src1.isRegister())
            {
                int rat_value_1 = ((GlobalData) globals).rat[src1.getRegisterNumber()];
                src1.rename(rat_value_1);
            }
            ...
            if (src2.isRegister())
            {
                int rat_value_2 = ((GlobalData) globals).rat[src2.getRegisterNumber()];
                src2.rename(rat_value_2);
            }

            if (oper0.isRegister())
            {
                if (oper0IsSource(opcode))
                {
                    int rat_value_oper0 = ((GlobalData)globals).rat[oper0.getRegisterNumber()];
                    oper0.rename(rat_value_oper0);
                }
            }
...
//                            Logger.out.println("Stall JMP wants oper0 R" + oper0.getRegisterNumber());
                    ...
                    int old_reg = ((GlobalData)globals).rat[oper0.getRegisterNumber()];
                    regfile.markRenamed(old_reg, true);
                    regfile.changeFlags(reg_ntu, (SET_USED | SET_INVALID), (CLEAR_FLOAT | CLEAR_RENAMED));
                    ((GlobalData)globals).rat[oper0.getRegisterNumber()] = reg_ntu;
                    oper0.rename(reg_ntu);
...
//                    return;
            ...
            if (opcode == EnumOpcode.MUL) {
                ...
                output_num = lookupOutput("DecodeToIMul");
            ...
            else if (opcode == EnumOpcode.FSUB) {
                output_num = lookupOutput("DecodeToFAddSub");
                output = this.newOutput(output_num);
            }
            ...
            if (oper0.isRegister())
            {
                if (opcode.needsWriteback())
                {
                    if (reg_ntu != -1)
                    {
                        int reg_old = ((GlobalData)globals).rat[oper0.getRegisterNumber()];
                        regfile.markRenamed(reg_old, true);
                        regfile.changeFlags(reg_ntu, (SET_USED | SET_INVALID), (CLEAR_FLOAT | CLEAR_RENAMED));
                        ((GlobalData)globals).rat[oper0.getRegisterNumber()] = reg_ntu;
                        oper0.rename(reg_ntu);
                    }
